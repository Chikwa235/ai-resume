  ...
  interface Window {
    puter: {
      ...
      kv: {
        ...
        // NOTE: runtime might be `del` not `delete`. Keep types but handle both below.
...
interface PuterStore {
  ...
  kv: {
    ...
    delete: (key: string) => Promise<boolean | undefined>; // our wrapper supports delete/del/remove
...
export const usePuterStore = create<PuterStore>((set, get) => {
  ...
  // ✅ FIXED: support runtime kv.del / kv.delete / kv.remove
  const deleteKV = async (key: string) => {
    ...
    if (typeof kvAny.delete === "function") return kvAny.delete(key);
    if (typeof kvAny.del === "function") return kvAny.del(key);
    if (typeof kvAny.remove === "function") return kvAny.remove(key);

    throw new Error("Puter KV has no delete/del/remove method");
  };
